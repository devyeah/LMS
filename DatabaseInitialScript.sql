...
IF NOT EXISTS (SELECT NULL FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Course' AND COLUMN_NAME = 'ScreenCast')
  ALTER TABLE dbo.Course
    ADD [ScreenCast] NVARCHAR(1000) NOT NULL DEFAULT ('https://res.cloudinary.com/funfood/image/upload/v1565220505/courses/default_cover.gif')
  GO

IF NOT EXISTS (SELECT NULL FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Avatar' AND COLUMN_NAME = 'AvatarUrl')
  ALTER TABLE dbo.Avatar
    ADD [AvatarUrl] NVARCHAR(1000) NOT NULL
  GO

IF NOT EXISTS (SELECT NULL FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Category' AND COLUMN_NAME = 'Icon')
  ALTER TABLE dbo.Category
    ADD [Icon] NVARCHAR(1000) NOT NULL
  GO

IF NOT EXISTS (SELECT NULL FROM sys.indexes WHERE NAME='IX_Account_Email')
